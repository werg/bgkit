...
class CommitEncodingTrainer(BaseTrainer):
    ...
    def setup(self) -> None:
        ...
        # First-time calibration: only fires when no checkpoint will load
        # (cold start). On resume, ``load_checkpoint`` overwrites T from the
        # saved buffer; we trust that saved T because the calibration
        # heuristic (std over 4 probe batches) is too noisy to override a
        # working saved value. Diagnosed 2026-05-08: across two resumes,
        # fresh calibration gave T=13.94 then T=107.50 from the same model
        # state — different probe batches produce wildly different mean
        # std. Saved T from a healthy run is more reliable.
    ...
    def _dynamic_ckpt_managed_models(self):
        """Register the three Step-5 backbones (encoder.l0, encoder.l1,
        decoder) with the memory-driven dynamic ckpt scheduler so it can
        flip their checkpointing mode in lockstep as system memory varies.
        """
        return [
