...
def main(cfg: DictConfig) -> None:
    ...
    if not cfg.get("analyze", None) or not cfg.analyze.get("checkpoint", None):
        raise ValueError("Pass +analyze.checkpoint=<path> +analyze.output_dir=<path>")
    if not cfg.analyze.get("output_dir", None):
        raise ValueError("Pass +analyze.output_dir=<path>")
    phase = cfg.training.get("phase", None)
    if phase not in ("phase1_step3", "phase1_step4"):
        raise ValueError(
            f"expects phase=phase1_step3 or phase1_step4, got {phase}. "
            "Use +experiment=phase1_step4 (or step3).",
        )
