#!/usr/bin/env python
"""Build a NewsQA provenance JSONL for ``scripts/build_teacher_trajectories.py``.

The Phase 2 ``newsqa`` mmap is built by ``scripts/convert_hf_to_mmap.py``
from the ``cnn_dailymail`` HF dataset (this is the surrogate NewsQA feed
used during Phase 2 — the real NewsQA dataset is gated and requires
per-article licence acceptance). Each row corresponds to one article and
carries a question/answer pair derived from the article's ``highlights``.

There is no hierarchy of news categories in the mmap (cnn_dailymail has
no usable category metadata), so the trajectory framework treats the
full news corpus as a single flat scope. The browse tree built by
``scripts/build_browse_tree.py --dataset newsqa`` is a one-level tree
over article ids.

Row schema emitted:
  - ``question``:         decoded from question_tokens.npy
  - ``gold_answer``:      decoded from answer_tokens.npy
  - ``gold_article_id``:  the row's own ``document_id`` (each article is
                          its own gold — NewsQA questions are
                          article-scoped, not corpus-scoped)
  - ``scope_template``:   ``"pre_scoped"``
  - ``scope_description``: ``"NewsQA news article corpus"``
"""
...
def _default_mmap_dir() -> Path | None:
    try:
        ...
        return get_data_dir(must_exist=False) / "mmap" / "phase2" / "newsqa"
    except Exception:
        return None
...
_SCOPE_DESCRIPTION = "NewsQA news article corpus"
...
def build_provenance(
    ...
    """Emit provenance JSONL rows for the NewsQA mmap at ``mmap_dir``.
    ...
    if "document_id" not in schema.names:
        raise RuntimeError(
            f"NewsQA metadata at {meta_path} is missing 'document_id'. "
    ...
    if len(q_offsets) - 1 != n_rows or len(a_offsets) - 1 != n_rows:
        raise RuntimeError(
            f"NewsQA mmap at {mmap_dir} has mismatched parallel row counts",
        )
    ...
    with output.open("w") as f:
        for idx, row in enumerate(metadata):
            ...
            if not doc_id:
                logger.warning(
                    "newsqa row %d missing document_id — skipping", idx,
                )
                n_skipped += 1
                continue
            ...
            if not question_ids or not answer_ids:
                logger.warning(
                    "newsqa row %d has empty question or answer tokens — skipping",
                    idx,
                )
                n_skipped += 1
                continue
    ...
    logger.info("newsqa provenance: wrote=%d skipped=%d", n_written, n_skipped)
    return n_written
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Phase 2 NewsQA mmap dir (default: $DATA_DIR/mmap/phase2/newsqa)",
    )
    parser.add_argument(
        "--output",
        type=Path,
