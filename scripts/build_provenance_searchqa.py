#!/usr/bin/env python
"""Build a SearchQA provenance JSONL for ``scripts/build_teacher_trajectories.py``.

The Phase 2 ``searchqa`` mmap is built by ``scripts/convert_hf_to_mmap.py``
from ``lucadiliello/searchqa`` — a drop-in replacement for the original
``search_qa`` dataset whose loading script was deprecated. Each HF row
represents one (question, 50 web snippets, answer) triple where the
snippets are pre-concatenated into a single ``context`` string.

The per-snippet gold identity is lost in that concatenation (the
converter writes the full joined context as one document blob and keeps
only the row's ``key`` as ``document_id``), so at the provenance level
the only gold article we can emit is the query_id itself. The browse
tree built by ``scripts/build_browse_tree.py --dataset searchqa`` is a
one-level flat tree over query_ids so the trajectory framework produces
a single ``bgkit`` call against the pre-scoped SearchQA corpus.

Row schema emitted:
  - ``question``:         decoded from question_tokens.npy
  - ``gold_answer``:      decoded from answer_tokens.npy
  - ``gold_article_id``:  the row's own ``document_id`` (the HF ``key``)
  - ``scope_template``:   ``"pre_scoped"``
  - ``scope_description``: ``"SearchQA web search snippets"``
"""
...
def _default_mmap_dir() -> Path | None:
    try:
        ...
        return get_data_dir(must_exist=False) / "mmap" / "phase2" / "searchqa"
    except Exception:
        return None
...
_SCOPE_DESCRIPTION = "SearchQA web search snippets"
...
def build_provenance(
    ...
    """Emit provenance JSONL rows for the SearchQA mmap at ``mmap_dir``.
    ...
    if "document_id" not in schema.names:
        raise RuntimeError(
            f"SearchQA metadata at {meta_path} is missing 'document_id'. "
    ...
    if len(q_offsets) - 1 != n_rows or len(a_offsets) - 1 != n_rows:
        raise RuntimeError(
            f"SearchQA mmap at {mmap_dir} has mismatched parallel row counts",
        )
    ...
    with output.open("w") as f:
        for idx, row in enumerate(metadata):
            ...
            if not doc_id:
                logger.warning(
                    "searchqa row %d missing document_id — skipping", idx,
                )
                n_skipped += 1
                continue
            ...
            if not question_ids or not answer_ids:
                logger.warning(
                    "searchqa row %d has empty question or answer tokens — skipping",
                    idx,
                )
                n_skipped += 1
                continue
    ...
    logger.info("searchqa provenance: wrote=%d skipped=%d", n_written, n_skipped)
    return n_written
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Phase 2 SearchQA mmap dir (default: $DATA_DIR/mmap/phase2/searchqa)",
    )
    parser.add_argument(
        "--output",
        type=Path,
