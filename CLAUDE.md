...
|---|---|---|---|---|
| ICE → Step 4 | `training.ice.checkpoint_path` | `ice` | `phase1_step4` | `eval/mse` |
...
| Step 2 → Step 3 | `step1_checkpoint` | `phase1_step2` | `phase1_step3` | `eval/loss` |
| Step 3 → Step 4 | `step1_checkpoint` | `phase1_step3` | `phase1_step4` | `eval/loss` |

**Training phase pipeline**: ICE → Joint Block Pretrain → Step 1 (frozen encoder pretraining) → Step 2 (DeltaNet pruning distillation) → Step 3 (commit encoding) → Step 4 (multi-objective compression) → Phase 2a (model ladder distillation) → Phase 2b (trajectory distillation) → Phase 2c (end-to-end injection with Qwen3.5-35B).
