...
**Compression-aware dynamic gradient-checkpointing (CommitEncoding, opt-in)**: when `training.dynamic_ckpt.enabled: true` is set in `phase1_step5.yaml`, the trainer downshifts the ckpt mode (`full` → `megatron` → `off`) as the median `mean_rate_l0` over a sliding window crosses thresholds. Snap-back to `full` when system memory crosses `memory_snap_back_gb`. Keys: `window`, `full_to_megatron_ratio`, `megatron_to_off_ratio`, `memory_snap_back_gb`, `min_steps_in_mode`. Implementation: `set_gradient_checkpointing_mode()` in `bgkit.training.gradient_utils` — runtime-mutable mode flip across encoder L0/L1 + decoder backbones. **Disabled by default** until the θ controller has been observed to track `curriculum_target_ratio_l0`; otherwise the scheduler stays in `full` mode (median ratio never crosses thresholds).
