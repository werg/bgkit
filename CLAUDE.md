...
**Per-level selection mode (Phase 2 `KRKBTrainer`, 2026-08-22)**: `training.selection_mode: {l0: threshold|exact_topk, l1: ...}` is parsed UNCONDITIONALLY (the legacy `recursive_l1_tree.selection_mode_l1` is honored as the L1 fallback — it used to be read only when the recursive tree was enabled, which silently left every widenet run in threshold mode at both levels). `exact_topk` realizes the configured ratio deterministically (`ceil(N·r) ≥ 1`), applies to the flat retrieval leaf's live L0 as well as tree leaves, skips that level's dual-ascent θ step, and disables θ-anchor probing in the retention sampler (`anchor_sampling_prob` → 0 for that level; anchor draws only calibrate θ(r) and would splice up to ~0.95·N reps). Resolved modes are logged at setup as `phase2_kb_selection_modes`. **Before launching any new Phase-2 data/routing, run `scripts/diag_flat_splice_counts.py +experiment=<exp> +diag.n_samples=8` in the GPU container**: it runs the real `_prepare_l1_turn`/`_run_l1_batch` on train samples and prints L0 keep rate, rep count / N and decoder length per sample — the 2026-08-22 audit found three silent routing defects (zero reps from `is_head` misrouting, a `--leaf-cap`-truncated browse tree, ignored selection modes) that every metric-level check had missed.

**`exact_topk` operator score = per-document z-score of `base_raw`** (`_segment_zscore`, 2026-08-22), exported as `logits_for_op` to the ranking AND every aux loss; the tanh-squashed score remains the `threshold` operator's (θ ∈ (−1,1) is defined against it). Why, in order of discovery on v5b: (1) a head at a tanh rail collapses the squashed score to ties → top-k arbitrary → answer-span survival at the chance rate while the span loss said the head had learned the span; (2) losses defined on the squashed score have no gradient at the rails (the aggregate-ratio term drove every token to tanh = −1 and all head losses froze for 130 steps); (3) an unsquashed `base_raw / T` lets the head satisfy ratio/span losses by inflating its output scale (std 2.96 → 120 → 1030) instead of re-ordering. Standardizing per document keeps the ordering, removes the scale degree of freedom, and keeps the loss probabilities O(1). Under exact_topk the temperature `T` is only a legacy normalizer; `KRKBTrainer._pre_train_loop` still re-estimates it on resume for exact_topk levels (harmless) and the calibration probe uses the first retrieval turn's article tokens. Related observability in `KRKBTrainer`: `loss/{family}` per decoder family, `grad_norm/<group>` pre-clip norms every `training.grad_norm_groups_every` steps (default 25; `survive_embedding` is its own group — its gradient is a sum over every survivor position and dominated the global norm), `eval/family/{family}/*`, and `training.eval_train_subset_samples: N` → `eval/train_subset/*` (the same teacher-forced pass over a fixed slice of training samples = generalization-gap probe per family). `training.grad_clip_mode: per_group` exists (each `_grad_norm_param_groups` group clipped on its own norm) but is opt-in: Adam/Muon updates are invariant to a uniform pre-clip scale, so global clipping does not starve small heads by itself. Under round-robin the eval's decoder family per sample follows the TRAINING mix (`KRKBTrainer._eval_family_for_index`: `qwen_decoder_prob` ≥1 → Qwen only, ≤0 → Falcon only, else alternate) — a Qwen-only run must not pool an untrained Falcon into `eval/loss`.

**Chat-template contract (2026-08-23)**: each decoder family is trained on its OWN template rendering — Qwen3.5 renders tool calls as XML (`<tool_call>\n<function=bgkit>\n<parameter=ids>…`) behind an empty `<think>\n\n</think>` scaffold; Falcon-H1 renders JSON and closes turns with `<|im_end|>` although its `eos_token` is `<|end_of_text|>`. Everything that consumes generations is derived from the template, never hard-coded: `parse_bgkit_call` accepts both renderings (fail-closed); `chat_template.end_of_turn_token_ids` discovers the stop token (`generate_with_single_splice` stops on it); `tokenize_trajectory` splits each assistant turn into scaffold | content | end-of-turn via `assistant_turn_scaffold` (marker rendering) — loss covers content + end-of-turn emission only, `answer_span`/`bgkit_call_spans` are content-only, and `generate_kb_turn` strips the end-of-turn glue. Flat rows name their document in scope (``"...; entrypoint id: `<doc_id>`"``, backtick-quoted because file paths may contain spaces; `flat_trajectory_row`) because `SYSTEM_PRE_SCOPED` tells the model to start from the id named there; `scope_entrypoint_ids` seeds the free-running evaluator. `tests/unit/data/test_bgkit_tool_template.py` runs on both real tokenizers and round-trips the decoded call span through the parser — keep it green. The in-training generative gate is `training.eval_free_running_samples: N` → `eval/kb/free_running/{<dataset>,<family>}/answer_exact_match`, `…/invalid/<reason>`, plus `free_running_example` log lines with raw generations. Before that date the "generative needle EM 0.000" of v5b was 100% measurement defects (JSON-only parser, Falcon never stopping, scaffold in spans, ids the prompt never showed).

**Teacher-forced eval runs decoders in train mode**: `KRKBTrainer._teacher_forced_decoders()` wraps every teacher-forced eval pass (headline, ablation sweep, train-subset probe, and `scripts/eval_phase2_kb.py`'s `evaluate_sample`) — Falcon-H1's `.eval()` Mixer path is numerically different (unfused + padding masking) and produced eval/loss 9.8 vs train ~1 with a zero reps-ablation gap; generation passes keep eval mode for the cache.

**Dynamic-ckpt allowed modes**: `BaseTrainer._dynamic_ckpt_allowed_modes()` declares which gradient-checkpointing modes the memory-driven scheduler may flip a trainer's managed models into (default: all). A disallowed transition is routed to the nearest allowed mode in the same direction or skipped (`ckpt_mode_transition_skipped`); the adaptive cache-flush tier is unaffected. `KRKBTrainer` declares `{full}`: its level forwards run under an OUTER non-reentrant `torch.utils.checkpoint`, and `megatron`'s inner selective policy breaks the outer recompute-match (`CheckpointError 58-vs-55`). Never disable `dynamic_ckpt` per run to dodge a flip — restrict the modes at the trainer.
...
**`shared_frac` alone does not separate healthy from collapsed.** The Phase-1 base sits at 0.927 with reps that identify their document at top-1 0.898; v8 sits at 0.990 at top-1 0.031; raw token embeddings at 0.225. A document's survivors sharing a strong common direction is normal — what mattered was whether that direction is the SAME for every document (`doc_mean_cos_to_corpus_mean`: base 0.961, v8 1.00000). The splice guard therefore also logs `mean_cos_to_corpus`, the cosine between each payload's mean and a running EMA of payload means.
...
Blocks are keyed by **phase** (`training.phase`, e.g. `phase2_kb`) and optionally by **run name** (`run_name`, e.g. `phase2_kb_widenet_v6`); a run reads both and the run block overrides the phase block key-wise. The phase block persists across runs of the same phase, so a fresh process logs every phase-block key it inherits at WARNING (`live_config_phase_block_inherited`) — check that line at launch; a stale block silently overrode v6's `max_steps` on 2026-08-23. Put run-specific knobs in the run-name block.
...
Defaults in `configs/compute/dgx_spark.yaml` under `memory.dynamic_ckpt`: `enabled: true`, `flush_when_free_below_gb: 20`, `flush_min_slack_gb: 3`, `megatron_upshift_when_free_below_gb: 15`, `full_upshift_when_free_below_gb: 8`, `downshift_margin_gb: 5`, `window: 50`, `min_steps_in_mode: 50`. Trainers opt in to mode-flips by overriding `_dynamic_ckpt_managed_models() → [(label, model), ...]`; default returns `[]` (only adaptive flush fires). `CommitEncodingTrainer` registers L0 + L1 encoder backbones + decoder; `KRKBTrainer` (all Phase 2 KB runs — Stage A/B + recursive-L1) likewise registers L0 + L1 backbones + the active decoder(s) (round-robin registers both Qwen3.5 + Falcon-H1 families) — **wired 2026-06-29; before that KRKBTrainer returned `[]` and was silently flush-only (mode-flip never armed), an oversight given it's a memory-heavy trainer.** Per-phase override via `training.dynamic_ckpt.{enabled,...}`. Implementation: `BaseTrainer._init_dynamic_ckpt_scheduler` / `_dynamic_ckpt_step` / `_apply_ckpt_mode`; mode-flip via `set_gradient_checkpointing_mode()` in `bgkit.training.gradient_utils`. Logs: `cuda_cache_cleared_adaptive` (per flush), `ckpt_mode_transition` (per mode change), `dynamic_ckpt_scheduler_armed` (once at startup).
